...
import { Plus, Search, BookOpen, Edit, Eye, Archive, CheckCircle, Clock } from 'lucide-react';
...
export const ContentManager: React.FC = () => {
  ...
  // Alterar Status do Ebook (Requisito 3.3) [cite: 87]
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-display font-bold">Gestão de Acervo</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full py-20 text-center text-text-muted italic">Carregando biblioteca tática...</div>
          ...
          <div key={book.id} className="bg-graphite-800 border border-graphite-700 rounded-xl overflow-hidden flex flex-col group hover:border-amber-500/30 transition-all shadow-lg">
            <div className="aspect-[16/9] relative overflow-hidden bg-black-900">
              ...
              <div className="absolute top-3 right-3">
                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-widest border ${
                  ...
                  book.status === 'draft' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 
                  'bg-graphite-600 text-text-muted border-graphite-500'
                }`}>
                  {book.status}
                </span>
              </div>
            ...
            <div className="bg-black-900/50 p-3 flex border-t border-graphite-700">
              ...
              <button 
                onClick={() => updateBookStatus(book.id, book.status === 'published' ? 'archived' : 'published')}
              ...
              >
                {book.status === 'published' ? <Archive size={14} /> : <CheckCircle size={14} />}
