  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-display font-bold text-text-primary uppercase tracking-tight">
          Gestão de Acervo
        </h2>
      ...
      <div className="relative w-full">
        ...
        <input
          type="text"
          placeholder="Buscar manual ou protocolo..."
          value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full py-20 text-center text-text-muted italic">
            Sincronizando biblioteca tática...
          </div>
        ) : filteredBooks.length === 0 ? (
          <div className="col-span-full py-20 text-center text-text-muted">
            Nenhum ebook encontrado no banco de dados.
          </div>
        ) : (
          filteredBooks.map((book) => (
            <div
              key={book.id}
              className="bg-graphite-800 border border-graphite-700 rounded-xl overflow-hidden flex flex-col group hover:border-amber-500/30 transition-all shadow-lg"
            >
              ...
              <div className="p-5 flex-1 flex flex-col">
                <span className="text-amber-500 text-[10px] font-bold uppercase tracking-widest mb-1">
                  {book.category}
                </span>
                <h3 className="text-lg font-bold leading-tight mb-2 text-text-primary line-clamp-1">
                  {book.title}
                </h3>
              ...
              <div className="bg-black-900/50 p-3 flex border-t border-graphite-700">
                ...
                <button
                  onClick={() =>
                    updateBookStatus(book.id, book.status === 'published' ? 'draft' : 'published')
                  }
